...
const { getWordPackService, getSentencePackService } = require('../services/utilsServices');
...
exports.getSentencePackMatch = async (req, res, next) => {
  try {
      let { nQuestion = 50, ...packInfo } = req.query;
      nQuestion = parseInt(nQuestion);
      if (nQuestion > MAX.LEN_SENTENCE_PACK) nQuestion = MAX.LEN_SENTENCE_PACK;

      const seedList = await getSentencePackService(packInfo, 0, 1500, '-_id sentence mean');
      if (seedList) {
          return res.status(200).json({
              sentencePack: seedList.sort((_) => Math.random() - 0.5).slice(0, nQuestion),
          });
      }

      return res.status(200).json({ sentencePack: [] });
  } catch (error) {
      console.error('GET SENTENCE PACK ERROR: ', error);
      return res.status(503).json({ message: 'Lỗi dịch vụ, thử lại sau' });
  }
};
