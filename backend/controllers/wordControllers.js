const {
  ...
  uploadImage,
  getWordPackService,
} = require('../services/utilsServices');
...
exports.getWordsPack = async (req, res) => {
  try {
...
exports.getUserFavoriteList = async (req, res, next) => {
  try {
    ...
    let { page = 1, perPage = 10, sortType = 'asc' } = req.query;
    page = parseInt(page);
    ...
    return res.status(200).json({ listFavorite: packList });
  } catch (error) {
