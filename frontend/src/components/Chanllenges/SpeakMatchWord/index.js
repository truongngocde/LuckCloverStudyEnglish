...
function WordMatch({ list }) {
  ...
  useEffect(() => {
    if (text.toLowerCase() === list[current]?.word.toLowerCase()) {
      handleCorrect();
    } else if (text !== '') {
      handleWrong();
    }
          ...
          <>
            ...
            {list && list.length > 0 ? (
              <div className="flex-center-col">
                {current < nQuestion ? (
                  <>
                    <p>Hãy đọc từ: <b>{list[current].mean}</b></p>
            ...
            {current < nQuestion && (
              <Tooltip title={current < nQuestion - 1 ? 'Qua câu kế' : 'Kết thúc'}>
                <div
